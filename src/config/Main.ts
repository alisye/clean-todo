...
export async function main(): Promise<void> {
  ...
  const getTodosController = ControllersConfiguration.getTodosController(
    getTodosUseCase
  );
  const createTodoController = ControllersConfiguration.getCreateTodoController(
    createTodoUseCase
  );
  const updateTodoController = ControllersConfiguration.getUpdateTodoController(
    updateTodoUseCase
  );

  const todoRouter = RouterConfiguration.getTodoRouter(
    getTodosController,
    createTodoController,
    updateTodoController
  );
  const expressApp = WebAppConfiguration.getExpressApp(
    { port: 3000 },
    todoRouter
  );
